...
import ug.co.absa.microsrvc.utility.internal.IntegrationTest;
...
class AbsaUgdtMicroInternalUtilitySvcKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/absa-ugdt-micro-internal-utility-svc-kafka/publish?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/absa-ugdt-micro-internal-utility-svc-kafka/register"))
            .andExpect(status().isOk())
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/absa-ugdt-micro-internal-utility-svc-kafka/unregister"));
                return;
            }
        }
